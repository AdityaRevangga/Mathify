...
const Quiz = {
  async findByMaterialId(materialId) {
    const result = await db.query(
      `SELECT q.id, q.material_id, q.title, q.description, q.time_limit, q.passing_score, q.is_active, q.created_at,
              (SELECT COUNT(*) FROM quiz_questions WHERE quiz_id = q.id)::int as question_count
       FROM quizzes q WHERE q.material_id = $1 AND q.is_active = true`,
      [materialId]
    );
    return result.rows;
  },
