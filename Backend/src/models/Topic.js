...
const Topic = {
  async findAll() {
    const result = await db.query(
      `SELECT t.id, t.name, t.slug, t.description, t.icon_url, t.sort_order, t.is_active, t.created_at,
              (SELECT COUNT(*) FROM materials m WHERE m.topic_id = t.id) as material_count
       FROM topics t WHERE t.is_active = true ORDER BY t.sort_order ASC`
    );
    return result.rows;
  },
