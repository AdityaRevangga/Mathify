...
const Material = {
  async findByTopicId(topicId) {
    const result = await db.query(
      `SELECT id, topic_id, title, slug, description, sort_order, is_active, created_at
  ...
  async create({ topic_id, title, slug, description, sort_order }) {
    const result = await db.query(
      `INSERT INTO materials (topic_id, title, slug, description, sort_order)
       VALUES ($1, $2, $3, $4, $5) RETURNING *`,
      [topic_id, title, slug, description, sort_order || 0]
    );
    return result.rows[0];
  },

  async update(id, { title, slug, description, sort_order, is_active }) {
    const result = await db.query(
